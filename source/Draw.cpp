...
void DrawFilledRect(int x, int y, int w, int h, uint8_t colour)
{
	for (int j = 0; j < h; j++)
	{
		for (int i = 0; i < w; i++)
...
void DrawRect(int x, int y, int w, int h, uint8_t colour)
{
	for (int j = 0; j < w; j++)
	{
...
const char PortStr[] PROGMEM = "NDS port by AzizBgBoss";
const char GithubStr[] PROGMEM = "https://github.com/AzizBgBoss";
...
void DrawStartScreen()
{
	...
	const int logoY = DISPLAY_HEIGHT / 2 - 31;
	const int spacing = 9;
	...
	int y = logoY + logoHeight - 2;
	int x = DISPLAY_WIDTH / 2 - FONT_WIDTH * 5;

	int cursorRectY = y + spacing * UIState.selection - 2;
	...
	y += spacing;
	DrawString(PortStr, x - FONT_WIDTH * 6, y);
	y += spacing;
	DrawString(GithubStr, x - FONT_WIDTH * 9, y);
}
...
{
	const int mapY = DISPLAY_HEIGHT / 2 - MAP_HEIGHT / 2 - 4;
	DrawFilledRect(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
